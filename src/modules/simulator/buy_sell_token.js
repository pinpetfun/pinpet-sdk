...
async function simulateTokenBuy(mint, buyTokenAmount, passOrder = null, lastPrice = null, ordersData = null) {
  ...
  if (!ordersData) {
    ...
    // Extract actual order array
    orders = ordersResponse.data.orders;
  } else {
    // If ordersData provided, extract order array from response and limit length
    orders = ordersData.data.orders.slice(0, this.sdk.MAX_ORDERS_COUNT + 1);
  }

  // Call calcLiqTokenBuy for liquidity calculation
  try {
    ...
    console.error('calcLiqTokenBuy call failed:', error);
    throw error;
  }
}
...
async function simulateTokenSell(mint, sellTokenAmount, passOrder = null, lastPrice = null, ordersData = null) {
  ...
  // Call calcLiqTokenSell for liquidity calculation
  try {
    ...
    console.error('calcLiqTokenSell call failed:', error.message);
    throw error;
  }
}
