 ...
 */
async function closeShortTokens(customParams = null) {
  const startTime = Date.now();
  
  try {
    BotGlobal.logMessage('info', '=== 开始执行平仓做空交易 ===');
    ...
    // 2. 获取平仓参数
    let closeShortParams = customParams || {};
    if (!customParams) {
      // 从配置中查找平仓做空计划
      const closeShortPlan = config.tradingPlan.find(plan => plan.type === 'closeShort' && plan.enabled);
      if (closeShortPlan && closeShortPlan.params) {
        closeShortParams = closeShortPlan.params;
      }
    }
    
    // 获取 orderBy 参数，默认为 'start_time_asc'（先平旧订单）
    const orderBy = closeShortParams.orderBy || 'start_time_asc';
    BotGlobal.logMessage('info', `使用订单排序方式: ${orderBy}`);
    
    // 获取 closeRate 参数，默认为 100.0（全部平仓）
    const closeRate = closeShortParams.closeRate || 100.0;
    ...
    // 3. 查找可平仓的做空订单
    BotGlobal.logMessage('info', '正在查找做空订单...');
    ...
    try {
      ...
      if (!userOrdersResult.success || !userOrdersResult.data.orders) {
        ...
        return {
          success: true,
          skipped: true,
          message: '没有找到可关闭的做空订单'
        };
      }
      
      // 查找做空订单 (order_type === 2)
      const shortOrders = userOrdersResult.data.orders.filter(order => order.order_type === 2);
      
      if (shortOrders.length === 0) {
        BotGlobal.logMessage('info', '没有找到做空订单，跳过平仓操作');
        return {
          success: true,
          skipped: true,
          message: '没有找到可关闭的做空订单'
      ...
      BotGlobal.logMessage('info', `找到 ${shortOrders.length} 个做空订单`);
      
      // 选择第一个做空订单进行平仓
      const targetOrder = shortOrders[0];
      ...
      const borrowAmount = actualCloseAmount; // 实际买入数量
      ...
      // 4. 设置平仓参数
      const buyTokenAmount = new anchor.BN(borrowAmount.toString());
      const maxSolAmount = new anchor.BN('100000000000'); // 设置一个较大的最大SOL支出，避免交易失败
      
      BotGlobal.logMessage('info', `平仓参数:`);
      BotGlobal.logMessage('info', `  - 买入代币数量: ${(Number(buyTokenAmount) / 1e6).toFixed(2)} 个`);
      BotGlobal.logMessage('info', `  - 最大SOL支出: ${(Number(maxSolAmount) / 1e9).toFixed(4)} SOL`);
      ...
      // 6. 构建平仓做空交易
      BotGlobal.logMessage('info', '开始构建平仓做空交易...');
      const closeShortResult = await sdk.trading.closeShort({
      ...
      BotGlobal.logMessage('info', '平仓做空交易已构建');
      BotGlobal.logMessage('info', '交易详情:');
      BotGlobal.logMessage('info', `  - 使用订单数: ${closeShortResult.orderData.ordersUsed}`);
      BotGlobal.logMessage('info', `  - LP配对数: ${closeShortResult.orderData.lpPairsCount}`);
      
      // 7. 签名并发送交易
      BotGlobal.logMessage('info', '开始签名和发送平仓做空交易...');
      
      // 设置交易参数
      closeShortResult.transaction.feePayer = wallet.keypair.publicKey;
      closeShortResult.transaction.recentBlockhash = (
        await connection.getLatestBlockhash()
      ).blockhash;
      
      // 签名交易
      closeShortResult.transaction.sign(wallet.keypair, ...closeShortResult.signers);
      
      // 发送交易
      const signature = await connection.sendRawTransaction(
        closeShortResult.transaction.serialize(),
        {
          skipPreflight: false,
      ...
      BotGlobal.logMessage('info', `平仓做空交易已发送，签名: ${signature}`);
      ...
      if (confirmation.value.err) {
        throw new Error('平仓做空交易失败: ' + JSON.stringify(confirmation.value.err));
      }
      ...
      BotGlobal.logMessage('info', `✅ 平仓做空成功！耗时 ${duration} 秒`);
      ...
      // 10. 更新全局状态 - 处理平仓后的空头仓位
      let currentShortPositions = state.state.positions.shortPositions || [];
      let updatedShortPositions;
      
      if (closeRate >= 100.0) {
        // 全部平仓，移除该仓位
        updatedShortPositions = currentShortPositions.filter(pos => pos.orderPda !== orderPda);
        const closedPositionsCount = currentShortPositions.length - updatedShortPositions.length;
        BotGlobal.logMessage('info', `已完全平仓，移除 ${closedPositionsCount} 个空头仓位`);
      } else {
        // 部分平仓，更新仓位数量（如果存在的话）
        updatedShortPositions = currentShortPositions.map(pos => {
      ...
      BotGlobal.setState('positions.shortPositions', updatedShortPositions);
      
      // 11. 添加交易历史
      BotGlobal.addTradeHistory({
        type: 'closeShort',
        description: `平仓做空 ${(Number(buyTokenAmount) / 1e6).toFixed(0)} 个代币 (${closeRate}%)`,
        status: 'completed',
      ...
      BotGlobal.logMessage('info', '=== 平仓做空完成 ===');
    ...
    BotGlobal.logMessage('error', `❌ 平仓做空失败 (耗时 ${duration} 秒): ${error.message}`);
  ...
  closeShortTokens()
    .then((result) => {
      if (result.skipped) {
        ...
        console.log('买入代币:', (Number(result.buyTokenAmount) / 1e6).toFixed(2), '个', `(${result.results?.closeRate || 100}%)`);
...
module.exports = { closeShortTokens };